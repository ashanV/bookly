...
import { format, addMinutes, isSameDay, addDays, isSunday, isBefore, startOfDay } from 'date-fns';
...
export default function ThreeDayView({ date, employees = [], reservations = [], draftVisit = null, onReservationClick, onEmptySlotClick, onViewChange, onEmployeeFilter }) {
    ...
    }, [employees.length]); // Re-run when view mode might change
    ...
    const handleSlotClick = (e, date, hours, minutes, employeeId) => {
        ...
        // Open Popover instead of direct action
        const x = e.clientX;
        const y = e.clientY;
    ...
    const handleEmployeeClick = (e, employeeId) => {
        e.stopPropagation(); // Stop propagation to avoid weird issues
                    ...
                    <div className="flex relative min-h-full">
                        ...
                        <div className="flex-1 flex relative">
                            {days.map((day, dayIndex) => {
                                ...
                                return (
                                    <div key={dayIndex} className={`flex-1 min-w-[150px] border-r border-gray-100 last:border-r-0 relative group z-10 ${isNonWorking ? 'bg-gray-50' : 'bg-white'}`}>
                ...
                <div className="flex-1 flex overflow-hidden">
                    {days.map((day, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`flex-1 min-w-[200px] p-4 text-center border-r border-gray-100 last:border-r-0 transition-colors duration-200
                                    ${isNonWorking ? 'bg-[#f3f4f6]' : 'bg-white'}
                            ...
                            <div key={employee._id} className="flex border-b border-gray-100 min-h-[150px]">
                                ...
                                {days.map((day, dayIndex) => {
                                    ...
                                    return (
                                        <div
                                            key={dayIndex}
                                            className="flex-1 border-r border-gray-100 relative p-2 min-w-[200px]"
                                            style={{
                                                backgroundColor:
